...
if __name__ == "__main__":
    ...
    # Debug data
    board_moment_widget = pyglet.shapes.Sector(x = 30, y = RESOLUTION - 30, radius = 20, angle = np.deg2rad(0), color = (255, 0, 0))
    rudder_moment_widget = pyglet.shapes.Sector(x = 100, y =  RESOLUTION - 30, radius = 20, angle = np.deg2rad(0), color = (0, 0, 255))
    sail_moment_widget = pyglet.shapes.Sector(x = 170, y =  RESOLUTION - 30, radius = 20, angle = np.deg2rad(0), color = (0, 255, 0))
    ...
    def step_physics(dt):
        ...
        boat_vector = [np.cos(boat_state.particle_state.theta), np.sin(boat_state.particle_state.theta)]
        ...
        board_moment_widget.angle = np.deg2rad(360 * debug_data["moments"]["board"] / 800)
        rudder_moment_widget.angle = np.deg2rad(360 * debug_data["moments"]["rudder"] / 800)
